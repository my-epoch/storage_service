...
// RegisterStorageServiceAPIHandlerServer registers the http handlers for service StorageServiceAPI to "mux".
// UnaryRPC     :call StorageServiceAPIServer directly.
...
// RegisterStorageServiceAPIHandlerFromEndpoint is same as RegisterStorageServiceAPIHandler but
// automatically dials to "endpoint" and closes the connection when "ctx" gets done.
func RegisterStorageServiceAPIHandlerFromEndpoint(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) (err error) {
	...
	return RegisterStorageServiceAPIHandler(ctx, mux, conn)
}

// RegisterStorageServiceAPIHandler registers the http handlers for service StorageServiceAPI to "mux".
// The handlers forward requests to the grpc endpoint over "conn".
func RegisterStorageServiceAPIHandler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn) error {
	return RegisterStorageServiceAPIHandlerClient(ctx, mux, NewStorageServiceAPIClient(conn))
}

// RegisterStorageServiceAPIHandlerClient registers the http handlers for service StorageServiceAPI
// to "mux". The handlers forward requests to the grpc endpoint over the given implementation of "StorageServiceAPIClient".
// Note: the gRPC framework executes interceptors within the gRPC handler. If the passed in "StorageServiceAPIClient"
...
func RegisterStorageServiceAPIHandlerClient(ctx context.Context, mux *runtime.ServeMux, client StorageServiceAPIClient) error {
	...
	mux.Handle("GET", pattern_StorageServiceAPI_GetRawFile_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		...
		inboundMarshaler, outboundMarshaler := runtime.MarshalerForRequest(mux, req)
		var err error
		ctx, err = runtime.AnnotateContext(ctx, mux, req, "/storage_service_api.StorageServiceAPI/GetRawFile", runtime.WithHTTPPathPattern("/api/v1/storage/{uuid}"))
		if err != nil {
			runtime.HTTPError(ctx, mux, outboundMarshaler, w, req, err)
			return
		}
		resp, md, err := request_StorageServiceAPI_GetRawFile_0(ctx, inboundMarshaler, client, req, pathParams)
		ctx = runtime.NewServerMetadataContext(ctx, md)
		if err != nil {
			runtime.HTTPError(ctx, mux, outboundMarshaler, w, req, err)
			return
